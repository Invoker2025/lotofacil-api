...
.history-row{display:grid;grid-template-columns:110px minmax(0,1fr) 64px;gap:12px;align-items:center;background:var(--panel2);border:1px solid var(--line);border-radius:8px;padding:10px}
.history-meta{display:grid;gap:3px}.history-contest{font-weight:800}.history-date{color:var(--muted);font-size:12px}
.history-numbers{display:flex;gap:5px;flex-wrap:wrap}.mini-ball{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;border:1px solid var(--line)}
.mini-ball.even{background:var(--green-bg);border-color:var(--green);color:#dcfce7}.mini-ball.odd{background:var(--red-bg);border-color:var(--red);color:#fee2e2}
.history-pattern{text-align:center;font-weight:900;color:var(--text);background:#172033;border:1px solid var(--line);border-radius:999px;padding:6px 8px}
.loading-text{color:var(--muted);font-size:13px}
@media(max-width:768px){.history-row{grid-template-columns:1fr;gap:8px;padding:12px}.history-pattern{justify-self:start}.history-numbers{gap:4px}.mini-ball{width:26px;height:26px;font-size:11px}}
@media(min-width:769px) and (max-width:980px){.history-row{grid-template-columns:96px minmax(0,1fr) 58px}}
@media(max-width:380px){.mini-ball{width:25px;height:25px;font-size:10px}}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div class="grid">
    ...
    <main>
      ...
      <div class="card"><div class="title">Últimos 10 concursos oficiais <span id="historyStatus" class="pill">Carregando</span></div><div id="historyList" class="history-list"><div class="loading-text">Buscando concursos oficiais...</div></div></div>
    </main>
  </div>
</div>
<script>
...
function renderHistory(draws){el('historyList').innerHTML=(draws||[]).map(d=>{const nums=d.numbers||[];const even=d.even_count??nums.filter(n=>n%2===0).length;const odd=d.odd_count??nums.filter(n=>n%2===1).length;const balls=nums.map(n=>`<span class="mini-ball ${n%2===0?'even':'odd'}">${pad(n)}</span>`).join('');return `<div class="history-row"><div class="history-meta"><div class="history-contest">Concurso ${d.contest}</div><div class="history-date">${d.date||'-'}</div></div><div class="history-numbers">${balls}</div><div class="history-pattern">${even}-${odd}</div></div>`}).join('');}
async function loadHistory(){el('historyStatus').innerText='Carregando';el('historyList').innerHTML='<div class="loading-text">Buscando concursos oficiais...</div>';try{const data=await api('/lotofacil',{limit:10});renderHistory(data.results||[]);el('historyStatus').innerText=`${data.count||0} jogos`;}catch(err){el('historyStatus').innerText='Erro';el('historyList').innerHTML=`<span class="error">${err.message||'Falha ao carregar histórico'}</span>`;}}
async function loadAll(force=false){const seq=++requestSeq;if(activeController)activeController.abort();activeController=new AbortController();const signal=activeController.signal;const {E,O}=readParity();const w=el('selWindow').value;setLoading(E,O);try{const p=await api('/parity',{window:w,even:E,odd:O,...(force?{force:true}:{})},signal);if(seq!==requestSeq)return;renderBalls('suggBalls',p.suggestion.combo);el('suggestionPattern').innerText=p.pattern||p.suggestion.pattern||`${E}-${O}`;el('gamesMetric').innerText=p.considered_games??'-';const backtestPath=`/backtest/latest?even=${E}&odd=${O}`;console.log('[APP] Backtest:',backtestPath);const b=await api('/backtest/latest',{even:E,odd:O},signal);if(seq!==requestSeq)return;const hits=new Set(b.hits||[]);renderBalls('autoSuggested',b.suggested,hits);renderBalls('autoOfficial',b.official,hits);el('contestBadge').innerText=`Concurso ${b.contest}`;el('hitsMetric').innerText=b.hits_count;el('patternMetric').innerText=b.pattern;el('currentPattern').innerText=b.pattern;el('autoResult').innerHTML=`<strong>${b.hits_count} acertos</strong><br><span>Paridade testada: <strong>${b.pattern}</strong></span><br><span>Concurso: <strong>${b.contest}</strong></span>`;setDone();}catch(err){if(err.name==='AbortError')return;setError(err.message||'Falha inesperada');}}
